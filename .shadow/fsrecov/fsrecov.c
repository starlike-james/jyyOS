...
u32 cluster_to_sec(u32 n) { return DataSec + (n - 2) * hdr->BPB_SecPerClus; }
...
u8 abs_u8(u8 a, u8 b) { return a > b ? a - b : b - a; }
...
void recover(u32 dataClus, const char *fname) {
    ...
    if (padding == 0) {
        ...
        // printf("clusterSize %u offset %u offsetSize %u ", clusterSize,
        // bhr->offset, offsetSize);
        ...
        if (offsetSize >= bhr->filesize) {
            ...
            while (remain > 0) {
                ...
                if ((uintptr_t)(clus + clusterSize) >=
                    (uintptr_t)hdr + hdr->BPB_TotSec32 * hdr->BPB_BytsPerSec) {
                    flag = false;
                }

                // assert((uintptr_t)pp < (uintptr_t)hdr + hdr->BPB_TotSec32 *
                // hdr->BPB_BytsPerSec);
                ...
                if (flag) {
                    ...
                    for (int k = 0; k < ClusterCnt; k++) {
                        ...
                        // int *diff = malloc(rowPixel * sizeof(int));

                        struct pixel *prevPixel =
                            (struct pixel *)(clus + clusterSize) - rowPixel;
                        struct pixel *curPixel = (struct pixel *)addr;
                        for (int j = 0; j < rowPixel; j++) {
                            diff += abs_u8(curPixel->red, prevPixel->red) +
                                    abs_u8(curPixel->green, prevPixel->green) +
                                    abs_u8(curPixel->blue, prevPixel->blue);
                            prevPixel++;
                            curPixel++;
                        }
...
void traverse_dir(u32 clusId) {
    ...
    for (int d = 0; d < ndents; d++) {
        struct fat32dent *dent =
            (struct fat32dent *)cluster_to_addr(clusId) + d;
        if (dent->DIR_Attr & ATTR_HIDDEN || dent->DIR_Name[0] == 0 ||
            ...
            for (int i = d - 1; i >= 0; i--) {
                int ord = d - i;
                struct fat32lfndent *ldent =
                    (struct fat32lfndent *)cluster_to_addr(clusId) + i;
                if ((ldent->LDIR_Attr & ATTR_LONG_NAME_MASK) !=
                    ATTR_LONG_NAME) {
                    break;
                }
                ...
                memcpy((u8 *)LDIR_Name + sizeof(ldent->LDIR_Name1),
                       ldent->LDIR_Name2, sizeof(ldent->LDIR_Name2));
                memcpy((u8 *)LDIR_Name + sizeof(ldent->LDIR_Name1) +
                           sizeof(ldent->LDIR_Name2),
                       ldent->LDIR_Name3, sizeof(ldent->LDIR_Name3));

                for (int k = 0; k < sizeof(LDIR_Name) / sizeof(LDIR_Name[0]);
                     k++) {
...
int main(int argc, char *argv[]) {
    ...
    DataSecCnt = hdr->BPB_TotSec32 - hdr->BPB_RsvdSecCnt -

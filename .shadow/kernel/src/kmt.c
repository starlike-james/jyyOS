...
static int task_create(task_t *task, const char *name, void (*entry)(void *arg), void *arg){
    ...
    task->context = *kcontext((Area){.start = task->stack, task->stack + STACK_SIZE },  task->entry, task->arg);

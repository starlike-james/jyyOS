...
void central_init(uintptr_t heap_start, uintptr_t heap_end) {
    block_t *head =
        (block_t *)(ROUNDUP((uintptr_t)heap_start, SLAB_PAGE)); // 64 KiB align
...
void *central_allocate(size_t size, bool slab) {
    ...
    for (; cur != NULL; prev = cur, cur = cur->next) {
        ...
        if (slab) {
            ...
            // (uintptr_t)cur, align); uintptr_t alloc_size = ROUNDUP(size,
            // SLAB_PAGE);
            ...
            size_t new_block_size =
                (uintptr_t)cur + cur->size - (ptr + alloc_size);
            if (new_block_size !=
                0) { // can not change to > 0 since it is unsigned ll
...
void central_free(void *ptr, bool slab) {
    ...
    *cursor = header;*/
    ...
    if (prev == NULL) {
        ...
        assert((uintptr_t)prev < (uintptr_t)header &&
               ((uintptr_t)prev + prev->size) <= (uintptr_t)header);
    }
    ...
    if (header->next != NULL) {
        ...
        assert((uintptr_t)header < (uintptr_t)cur &&
               ((uintptr_t)header + header->size) <= (uintptr_t)(header->next));
    }
